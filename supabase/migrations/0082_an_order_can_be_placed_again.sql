-- ---------------------------------------------------------------------------
-- 0082 — an order can be placed again. (regression from 0078)
-- ---------------------------------------------------------------------------
-- **Nobody has been able to place an order since 0078 shipped.** The tax rewrite
-- (BIZ-005) added one statement to `place_order`:
--
--     update _lines set taxable_value = line_total - discount_alloc;
--
-- correct SQL, and fatal here. Supabase loads **pg_safeupdate** for the roles
-- PostgREST switches into, so an `update` with no `where` clause raises
-- SQLSTATE **21000 — "UPDATE requires a WHERE clause"** for `authenticated`,
-- and for nobody else. The customer app only shows a database message when the
-- code is `P0001`; 21000 fell through to `OrderPlacementFailure`'s default, so
-- every customer saw "We couldn't place your order. Please try again." and the
-- real error was never written down anywhere.
--
-- The last order in the book is ZPQ-1044, 29 July.
--
-- **Why every check on 0078 missed it.** They all ran through `psql` as
-- `postgres`, and `postgres` does not load the extension — called that way the
-- function is word-for-word correct, which is exactly what makes this worth
-- writing down. A client-callable RPC is only proven by calling it over HTTP
-- with a real user's token. That is how this was found, and it is how the next
-- one will be.
--
-- One statement changes. The whole body is restated because `create or replace`
-- takes a whole body, and because the repo's copy of this function has drifted
-- from the database more than once — what follows is the **live** definition
-- with that single line fixed and nothing else touched. Grants survive
-- `create or replace` and are deliberately not restated.
--
-- Swept the rest of the schema for the same shape: `place_order` was the only
-- function callable by `authenticated` carrying a `where`-less `update`.
-- ---------------------------------------------------------------------------

CREATE OR REPLACE FUNCTION public.place_order(p_user_phone text, p_restaurant_id text, p_items jsonb, p_delivery_to text, p_payment_method text, p_delivery_lat double precision DEFAULT NULL::double precision, p_delivery_lng double precision DEFAULT NULL::double precision, p_coupon_code text DEFAULT NULL::text, p_payment_id text DEFAULT NULL::text, p_delivery_notes text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
