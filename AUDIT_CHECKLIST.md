...
- [x] **CUS-001** — Hard 25-order ceiling with no "load more". **Closed 2026-08-07.** `fetchOrders` takes `offset`/`limit` and uses `.range()`; `ordersProvider` is now an `AsyncNotifier` that appends pages, with a "Show older orders" footer. Three things worth keeping: the order is `created_at DESC, id DESC` because two orders written in the same millisecond made the sequence non-total and a row could appear on two pages or none; **no count is fetched** (`count: exact` on a policy-filtered table buys a second scan per page to answer a question the screen never asks) — a short page *is* the end signal; and a failed `loadMore` throws rather than entering `state`, so the pages already on screen survive it.<br><sub>Order history · UX · effort S</sub>
